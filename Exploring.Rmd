...
Our final step before actually plotting and visualing our results is making 
...
Injuries <- aggregate(x = list(Impact = CleanStormData$FATALITIES)
                             , by = list(EVENT_TYPE = CleanStormData$EVTYPE)
                             , FUN = sum)

Injuries <- Injuries[order(Injuries$Impact,
                           decreasing = TRUE),] 
...
At this point, we are finally ready to visualize our results! using the plotting
library "ggplot", we are going to create 2 charts. Letting us view the grouped
comparison of each weather related event against the corresponding damage metric

```{r echo=TRUE}

FatalitiesPlot <- barplot(x = head(Fatalities$EVENT_TYPE, 10)
                        , y = Fatalities$Impact)
